...
def fetch_price(producer, symbol):
    ...
    try:
        ...
        page = urllib.request.urlopen('http://www.nasdaq.com/symbol/AAPL/real-time').read()
        soup = BeautifulSoup(page);
        price = soup.find("div", {"id": "qwidget_lastsale"}).string
        clean_price = float(price.replace('$', ''))


        #price = random.randint(30, 120) #funciona
        #rsp = requests.get('https://finance.google.com/finance?q=%s&output=json') % symbol
        ...
        payload = ('[{"StockSymbol":"%s","LastTradePrice":%d}]' % (symbol,clean_price)).encode('utf-8')
